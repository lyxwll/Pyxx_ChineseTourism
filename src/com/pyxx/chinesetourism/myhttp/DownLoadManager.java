...
import com.pyxx.chinesetourism.utils.SystemBase;
...
public class DownLoadManager {
	...
	public DownLoadManager(String path, String targetFile,
	...
	public void download() {
		/*
		 * if(downListener != null) { downListener.onStartDownLoad(); } new
	...
	private void doDownload(String path, String targetFile) {
		try {
			HttpURLConnection httpConnection = (HttpURLConnection) new URL(path)
					.openConnection();
			...
			if (fileSize > 20 * 1024 * 1000) {// 如果文件远大于20M， 建议使用单线程下载
				...
				if (fileSize <= unitSize) {// 如果远程文件尺寸小于等于unitSize
					...
					downloadThreads[0] = new DownloadThread(path, targetFile,
							offset, fileSize);
					...
					for (int i = 0; i < threadCount; i++) {
						downloadThreads[i] = new DownloadThread(path,
								targetFile, offset, unitSize);
						downloadThreads[i].start();
						offset = offset + unitSize;
					}
					if (fileSize % unitSize != 0) {// 如果不能整除，则需要再创建一个线程下载剩余字节
						downloadThreads[threadCount] = new DownloadThread(path,
								targetFile, offset, fileSize - unitSize
										* threadCount);
	...
	private class DownloadThread extends Thread {
		 ...
		 * 
		 * */
		protected DownloadThread(String url, String file, long offset,
				long length) {
			this.url = url;
			this.fileName = file;
			this.offset = offset;
		...
		public void run() {
			try {
				...
				Logg.out("RANGE bytes=" + this.offset + "-"
